...
def get_cpe_ids(cve_product, version):
    ...
    for product in cve_product.split():
        ...
        cpe_id = 'cpe:2.3:a:{0}:{1}:{2}:{3}:*:*:*:*:*:*'.format(vendor, product, version, update)
...
python do_vigiles_check() {
    ...
    def run_checkcves(d, cmd, args=[]):
        ...
        vigiles_env['VIGILES_SUBFOLDER_NAME'] = _orig_env.getVar('VIGILES_SUBFOLDER_NAME', True) or ''
        ...
        conf_subfolder_name = d.getVar('VIGILES_SUBFOLDER_NAME', True)
...
def _validate_sbom_download_args(d):
    ...
    layerdir = d.getVar("VIGILES_LAYERDIR", True)
    scripts_dir = os.path.join(layerdir, "scripts")
...
python do_vigiles_download_sbom() {
    ...
    python_bin = d.getVar("PYTHON", True)
    layerdir = d.getVar('VIGILES_LAYERDIR', True)
