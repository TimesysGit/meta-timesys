...
def _get_cve_version(d):
    ...
    cve_v = d.getVar('CVE_VERSION', True )
...
def _get_lifecycle_info(d):
    ...
    release_date = (d.getVar("LIFECYCLE_RELEASE_DATE", True) or "").strip()
    if release_date:
        lifecycle["release_date"] = release_date

    end_of_life = (d.getVar("LIFECYCLE_END_OF_LIFE", True) or "").strip()
    if end_of_life:
        lifecycle["end_of_life"] = end_of_life

    level_of_support = (d.getVar("LIFECYCLE_SUPPORT_LEVEL", True) or "").strip()
    if level_of_support:
...
python tsmeta_get_image() {
    ...
    extra_dict = { key.lower(): (d.getVar(key, True ) or "") for key in extra_keys }
