...
if cooker.start():
    ...
    (latest_versions, preferred_versions) = bb.providers.findProviders(cooker.data, cooker.recipecaches[''], cooker.recipecaches[''].pkg_pn)
    allproviders = bb.providers.allProviders(cooker.recipecaches[''])
    ...
    layer_info = { lyr['name'] : dict(collection=lyr.get('collection', 'UNKNOWN'), remote=lyr['remote'], rev=lyr['revision'], branch=lyr['branch']) for lyr in get_layer_info(cooker) }
    ...
    for p in depgraph['pn']:
        ...
        if realfn[1] and realfn[0] in cooker.recipecaches[''].pkg_fn:
            continue
        ...
        for key,value in cooker.recipecaches[''].file_checksums[realfn[0]].items():
            patches = value.split()
